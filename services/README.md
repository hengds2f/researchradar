...
### `clustering_service.py` — Semantic Clustering Map

**What it does in plain English:**  
Takes one vector embedding per paper (the average of its section chunk embeddings), and turns them into an interactive 2-D scatter map where papers that discuss similar topics sit close together.

**Pipeline (in order):**

| Step | What Happens |
|---|---|
| 1. Aggregate | Chunk embeddings are averaged per paper and L2-normalised |
| 2. Reduce to 2-D | **UMAP** (preferred — preserves local structure) or **PCA via numpy SVD** (fallback, no extra deps) |
| 3. Cluster | **HDBSCAN** (preferred — handles uneven density, marks true outliers) or **KMeans** (fallback) or single-cluster for tiny corpora |
| 4. Enrich | Each point gets a cluster ID, label, colour, and cosine-similarity score to its cluster centroid |
| 5. Cache | Results are keyed by a SHA-256 fingerprint of sorted paper IDs; subsequent calls return instantly until a paper is added, deleted, or the session is cleared |

**Fallback chain:** The service degrades gracefully when optional packages are absent:
- `umap-learn` missing → PCA
- `hdbscan` missing → KMeans
- `scikit-learn` missing → single cluster with random 2-D layout

**Response shape returned to `app.py`:**
```json
{
  "points":   [{ "id", "title", "authors", "year", "x", "y",
                 "cluster_id", "cluster_label", "is_outlier",
                 "similarity_score", "color" }],
...
**Key class:** `ClusteringService`  
**Key method:** `compute(papers_meta, embeddings_map)` — idempotent; returns cached result when corpus hasn't changed  
**Triggered by:** `GET /api/clustering` in `app.py`
